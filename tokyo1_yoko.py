...
def concatenate_table(tables_list):
    ...
    concatenate_base = pd.DataFrame(
        concatenate_base.values[1:], columns=concatenate_base.iloc[0])
    # 連結したデータを返す
...
for i, tokyo in enumerate(tokyo_lists):
    ...
    for j, column in enumerate(column_list):
        if column != None:
            concatenated_tables.rename(
                columns={concatenated_tables.columns[j]: concatenated_tables.columns[j].replace("\n", "").replace(" ", "").replace("　", "").replace("※", "")}, inplace=True)
    ...
    # tokyo1の処理
    if i == 0:
        concatenated_tables.iloc[:, 1] = concatenated_tables.iloc[:, 1].replace(
            ",", "", regex=True).astype("int64")
        # 空白削除
        concatenated_tables.iloc[:, 0] = concatenated_tables.iloc[:, 0].replace(
            ' ', '', regex=True)
        # 横持変換
        category_data = {
        ...
        concatenated_tables_horizontal = pd.DataFrame(columns=[
            "主要カテゴリ", "主要カテゴリ売上高(千円)", "主要カテゴリ構成比(%)", "主要カテゴリ対前年増減(-)率(%)", "サブカテゴリ", "サブカテゴリ売上高(千円)", "サブカテゴリ構成比(%)", "サブカテゴリ対前年増減(-)率(%)", "主要カテゴリ総額売上高(千円)", "主要カテゴリ総額対前年増減(-)率(%)", "年月"])
        ...
        for month in range(12):
            # 各月のデータ抽出
            temp = concatenated_tables[
                concatenated_tables["年月"] == f"2023-{month+1:02}"]
            # tempのインデックスを項目名に設定
            temp.index = temp[""]
            ...
            for key, values in category_data.items():
                ...
                # リスト形式の値を1つ1つ取り出す
                for value in values:
                    concatenated_tables_horizontal.loc[
                        row_index, "主要カテゴリ"] = key
                    concatenated_tables_horizontal.loc[
                        row_index, "主要カテゴリ売上高(千円)"] = temp.loc[key, "売上高(千円)"]
                    concatenated_tables_horizontal.loc[
                        row_index, "主要カテゴリ構成比(%)"] = temp.loc[key, "構成比（％）"]
                    concatenated_tables_horizontal.loc[
                        row_index, "主要カテゴリ対前年増減(-)率(%)"] = temp.loc[key, "対前年増減(-)率(%)"]
                    if value == "":
                        concatenated_tables_horizontal.loc[
                            row_index, "サブカテゴリ"] = "-"
                        concatenated_tables_horizontal.loc[
                            row_index, "サブカテゴリ売上高(千円)"] = "-"
                        concatenated_tables_horizontal.loc[
                            row_index, "サブカテゴリ構成比(%)"] = "-"
                        concatenated_tables_horizontal.loc[
                            row_index, "サブカテゴリ対前年増減(-)率(%)"] = "-"
                    else:
                        concatenated_tables_horizontal.loc[
                            row_index, "サブカテゴリ"] = value
                        concatenated_tables_horizontal.loc[
                            row_index, "サブカテゴリ売上高(千円)"] = temp.loc[value, "売上高(千円)"]
                        concatenated_tables_horizontal.loc[
                            row_index, "サブカテゴリ構成比(%)"] = temp.loc[value, "構成比（％）"]
                        concatenated_tables_horizontal.loc[
                            row_index, "サブカテゴリ対前年増減(-)率(%)"] = temp.loc[value, "対前年増減(-)率(%)"]
                    concatenated_tables_horizontal.loc[
                        row_index, "主要カテゴリ総額売上高(千円)"] = temp.loc["総額", "売上高(千円)"]
                    concatenated_tables_horizontal.loc[
                        row_index, "主要カテゴリ総額対前年増減(-)率(%)"] = temp.loc["総額", "対前年増減(-)率(%)"]
                    concatenated_tables_horizontal.loc[
                        row_index, "年月"] = f"2023-{month+1:02}"
                    # 行インデックスの更新
                    row_index += 1
        concatenated_tables = concatenated_tables_horizontal
    # 出力先ファイルを指定
    # 出力のためフォルダ変更
    output_file = f"depart_task_confirm\\export2\\tokyo{i+1}.csv"
    # csv出力
